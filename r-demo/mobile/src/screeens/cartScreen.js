...
export default memo(function CartScreen({ match, location, history }) {
  const productId = match.params.id; //拿到添加购物车的商品id
  const qty = location.search ? Number(location.search.split("=")[1]) : 1; //拿到数量
